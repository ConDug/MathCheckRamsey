...
def generate(n, p, q, lower=0, upper=0, u_e_b=0, u_e_r=0, mpcf=0):
    ...
    for clique in itertools.combinations(vertices, p):
        ...
        with open(
            f"./constraints_temp_{n}_{p}_{q}_{lower}_{upper}_{u_e_b}_{u_e_r}_{mpcf}",
            "a",
        ) as f:  # p-cliques
    ...
    count, clause_count = cubic(
        n,
        math.comb(n, 2),
        f"constraints_temp_{n}_{p}_{q}_{lower}_{upper}_{u_e_b}_{u_e_r}_{mpcf}",
    )  # write cubic constraints to file and count their total variables, and num_cubic constriants
    # clause_count =0
    # count=math.comb(n,2)
    ...
    if lower > 0:
        for i in range(1, n + 1):
            deg_count, deg_clause = generate_degree_clauses(
                [edge_dict[key] for key in edge_dict if i in key],
                lower,
                upper,
                count,
                f"constraints_temp_{n}_{p}_{q}_{lower}_{upper}_{u_e_b}_{u_e_r}_{mpcf}",
    ...
    if u_e_b > 0:
        ...
        for i in range(1, n * (n - 1) // 2 + 1):
            X = set(range(1, n + 1)) - set(
                list(edge_dict.keys())[list(edge_dict.values()).index(i)]
            )  # select all vertices except on i'th edge
            edge_count, edge_clause = generate_triangle_clauses(
                X,
                u_e_b,
                count,
                f"constraints_temp_{n}_{p}_{q}_{lower}_{upper}_{u_e_b}_{u_e_r}_{mpcf}",
    ...
    if u_e_r > 0:
        ...
        for i in range(1, n * (n - 1) // 2 + 1):
            X = set(range(1, n + 1)) - set(
                list(edge_dict.keys())[list(edge_dict.values()).index(i)]
            )  # select all vertiecs expect on i'th edge
            edge_count, edge_clause = generate_triangle_clauses(
                X,
                u_e_r,
                count,
    ...
    if mpcf == 0:
        mtf_count, mtf_clause = max_pclique_free(
            n,
            p,
            count,
            f"constraints_temp_{n}_{p}_{q}_{lower}_{upper}_{u_e_b}_{u_e_r}_{mpcf}",
    ...
    edge_count, edge_clause = generate_edge_clauses(
        list(range(1, math.comb(n, 2) + 1)),
        80,
        80,
        count,
        f"constraints_temp_{n}_{p}_{q}_{lower}_{upper}_{u_e_b}_{u_e_r}_{mpcf}",
    ...
    )  # call a bash file to combine cubic constraints and 1st line of cnf file
    proc1.wait()
